...
class SettingsPage(QWidget):
  ...
  def init_ui(self):
    ...
    self.btn_backup_local.setStyleSheet(
        'background-color: #3182CE; color: white; padding: 8px 12px;'
    ...
    self.btn_restore_local.setStyleSheet(
        'background-color: #D69E2E; color: white; padding: 8px 12px;'
        ' border-radius: 4px; font-weight: bold;'
    )
    ...
    self.btn_sync_drive.setStyleSheet(
        'background-color: #805AD5; color: white; padding: 8px 12px;'
        ' border-radius: 4px; font-weight: bold;'
    )
  ...
  def make_local_backup(self):
    ...
    if file_path:
      QMessageBox.information(
          self, 'نجاح', f'تم إنشاء نسخة احتياطية بنجاح:\n{file_path}'
      )
    else:
  ...
  def sync_to_drive(self):
    ...
    if not file_path:
      QMessageBox.critical(
          self, 'خطأ', 'لم يتم العثور على ملف النسخة الاحتياطية.'
      )
      return
    ...
    QMessageBox.information(
        self,
        'تنبيه',
        'سيتم فتح المتصفح لتأكيد تسجيل الدخول إلى حساب Google Drive الخاص بك.',
    )
  ...
  def load_options(self):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT id, category, value FROM settings_options WHERE category = %s'
        ' ORDER BY id DESC',
        (category_code,),
    )
    ...
    for row_idx, row in enumerate(rows):
      ...
      btn_delete.clicked.connect(
          lambda checked, opt_id=row['id']: self.delete_option(opt_id)
      )
