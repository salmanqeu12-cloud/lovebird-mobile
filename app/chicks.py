...
class AddEditChickDialog(QDialog):
    def __init__(self, parent=None, chick_data=None):
        ...
        self.setWindowTitle("تعديل بيانات فرخ" if chick_data else "إضافة فرخ جديد")
    ...
    def init_ui(self):
        ...
        self.txt_hatch_month.setPlaceholderText("MM-YYYY أو YYYY-MM (مثال: 6-2026)")
        ...
        self.btn_save.setStyleSheet("background-color: #3182CE; color: white; padding: 8px 16px; border-radius: 4px;")
        self.btn_cancel.setStyleSheet("background-color: #E2E8F0; color: #2D3748; padding: 8px 16px; border-radius: 4px;")
    ...
    def load_options(self):
        ...
        try:
            ...
            cursor.execute("SELECT pair_number FROM pairs ORDER BY pair_number ASC")
            ...
            cursor.execute("SELECT value FROM settings_options WHERE category = 'chick_status'")
            statuses = [row[0] for row in cursor.fetchall()]
            self.combo_status.addItems(statuses if statuses else ["محتفظ به", "للبيع", "تم البيع", "نافق"])
        finally:
            if conn:
                conn.close()
    ...
    def populate_data(self):
        ...
        self.txt_ring_num.setReadOnly(True)
        
        p_idx = self.combo_pair_num.findText(self.chick_data['pair_number'] or "بدون")
        ...
        if s_idx >= 0: self.combo_status.setCurrentIndex(s_idx)
...
class ChicksPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.txt_search.setPlaceholderText("🔍 بحث برقم الحجل أو رقم الزوج...")
        ...
        self.table.setHorizontalHeaderLabels([
            "رقم الحجل", "رقم الزوج", "شهر الفقس", 
            "العمر الحالي", "اللون", "الطفرات", 
            "الجنس", "الحالة", "ملاحظات", "إجراءات"
        ])
    ...
    def load_filter_options(self):
        ...
        try:
            ...
            if statuses:
                ...
                self.combo_filter_status.addItems(["محتفظ به", "للبيع", "تم البيع", "نافق"])
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                if diff_months < 0:
                    ...
                    year_str = "سنة" if years == 1 else ("سنتين" if years == 2 else f"{years} سنوات")
                    if rem_months == 0:
                        return year_str
    ...
    def load_chicks(self):
        ...
        try:
            ...
            query = "SELECT ring_number, pair_number, hatch_month, color, mutations, gender, status, notes FROM chicks WHERE 1=1"
            params = []

            if search_text:
                query += " AND (ring_number LIKE ? OR pair_number LIKE ?)"
        ...
        for row_idx, row in enumerate(rows):
            ...
            self.table.setItem(row_idx, 1, QTableWidgetItem(str(row['pair_number'] or "بدون")))
            self.table.setItem(row_idx, 2, QTableWidgetItem(str(row['hatch_month'] or "")))
            self.table.setItem(row_idx, 3, QTableWidgetItem(age_text))
            self.table.setItem(row_idx, 4, QTableWidgetItem(str(row['color'] or "")))
            self.table.setItem(row_idx, 5, QTableWidgetItem(str(row['mutations'] or "")))
            self.table.setItem(row_idx, 6, QTableWidgetItem(str(row['gender'] or "")))
            self.table.setItem(row_idx, 7, QTableWidgetItem(str(row['status'] or "")))
            self.table.setItem(row_idx, 8, QTableWidgetItem(str(row['notes'] or "")))
            ...
            chick_data = dict(row)
            btn_edit.clicked.connect(lambda checked, data=chick_data: self.edit_chick(data))
            btn_delete.clicked.connect(lambda checked, r_num=row['ring_number']: self.delete_chick(r_num))
    ...
    def add_chick(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if not data['ring_number'] or not data['hatch_month']:
                QMessageBox.warning(self, "خطأ", "يجب إدخال رقم الحجل وشهر الفقس!")
                return
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (data['ring_number'], data['pair_number'], data['hatch_month'], 
                      data['color'], data['mutations'], data['gender'], data['status'], data['notes']))
                
                # 2. المزامنة التلقائية مع قائمة جميع الطيور
                source_text = f"إنتاج الزوج {data['pair_number']}" if data['pair_number'] else "إنتاج فرخ"
                cursor.execute("""
                ...
                """, (data['ring_number'], data['gender'], data['color'], data['mutations'], data['status'], source_text))
    ...
    def edit_chick(self, chick_data):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                cursor.execute("""
                    UPDATE chicks 
                    SET pair_number=?, hatch_month=?, color=?, mutations=?, gender=?, status=?, notes=?
                    WHERE ring_number=?
                """, (data['pair_number'], data['hatch_month'], data['color'], 
                      data['mutations'], data['gender'], data['status'], data['notes'], data['ring_number']))
