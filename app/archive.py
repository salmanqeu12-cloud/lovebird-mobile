...
class AddEditArchiveDialog(QDialog):
  ...
  def __init__(self, parent=None, archive_data=None):
    ...
    self.setWindowTitle(
        "تعديل سجل الأرشيف" if archive_data else "أرشفة طير جديد"
    )
  ...
  def load_options(self):
    ...
    cursor.execute(
        "SELECT value FROM settings_options WHERE category = 'archive_reason'"
    )
    rows = cursor.fetchall()
    reasons = [
        list(row.values())[0] if isinstance(row, dict) else row[0]
    ...
    self.combo_reason.addItems(
        reasons if reasons else ["بيع", "نفوق", "استبعاد"]
    )
  ...
  def populate_data(self):
    ...
    self.txt_color_mutations.setText(
        str(self.archive_data["color_mutations"] or "")
    )

    g_idx = self.combo_gender.findText(
        str(self.archive_data["gender"] or "")
    )
    if g_idx >= 0:
      self.combo_gender.setCurrentIndex(g_idx)

    r_idx = self.combo_reason.findText(
        str(self.archive_data["reason"] or "")
    )
    if r_idx >= 0:
      self.combo_reason.setCurrentIndex(r_idx)

    if self.archive_data["archive_date"]:
      self.date_archive.setDate(
          QDate.fromString(self.archive_data["archive_date"], "yyyy-MM-dd")
      )
...
class ArchivePage(QWidget):
  ...
  def init_ui(self):
    ...
    self.txt_search.setPlaceholderText(
        "🔍 بحث برقم الحجل، اللون/الطفرات، أو الملاحظات..."
    )
  ...
  def load_archive(self):
    ...
    selected_reason = (
        self.combo_filter_reason.currentText()
        if hasattr(self, "combo_filter_reason")
        else "جميع الأسباب"
    )
    selected_gender = (
    ...
    query = (
        "SELECT id, ring_number, color_mutations, gender, reason, archive_date,"
        " notes FROM archive WHERE 1=1"
    )
    params = []

    if search_text:
      query += (
          " AND (ring_number LIKE %s OR color_mutations LIKE %s OR notes LIKE"
          " %s)"
      )
    ...
    for row_idx, row in enumerate(rows):
      ...
      self.table.setItem(
          row_idx, 1, QTableWidgetItem(str(row["color_mutations"] or ""))
      )
      self.table.setItem(
          row_idx, 2, QTableWidgetItem(str(row["gender"] or ""))
      )
      self.table.setItem(row_idx, 3, QTableWidgetItem(str(row["reason"])))
      self.table.setItem(
          row_idx, 4, QTableWidgetItem(str(row["archive_date"]))
      )
      self.table.setItem(
          row_idx, 5, QTableWidgetItem(str(row["notes"] or ""))
      )
      ...
      btn_restore.clicked.connect(
          lambda checked, data=archive_data: self.restore_bird(data)
      )
      btn_edit.clicked.connect(
          lambda checked, data=archive_data: self.edit_archive(data)
      )
      btn_delete.clicked.connect(
          lambda checked, a_id=row["id"]: self.delete_archive(a_id)
      )
  ...
  def restore_bird(self, archive_data):
    ...
    confirm = QMessageBox.question(
        self,
        "تأكيد الاسترجاع",
        f"هل أنت متأكد من إلغاء أرشفة الطير [{ring_num}] وإعادته إلى قائمة جميع"
        " الطيور؟",
      ...
      try:
        ...
        cursor.execute(
            """
                    ...
                    VALUES (%s, %s, %s, 'متاح', 'مسترجع من الأرشيف', %s)
        ...
        cursor.execute(
            "DELETE FROM archive WHERE id = %s", (archive_data["id"],)
        )
        ...
        QMessageBox.information(
            self,
            "تم الاسترجاع",
            f"تم استرجاع الطير [{ring_num}] وإعادته لقائمة جميع الطيور بنجاح.",
        )
        self.load_archive()
  ...
  def edit_archive(self, archive_data):
    ...
    if dialog.exec() == QDialog.Accepted:
      ...
      cursor.execute(
          """
                UPDATE archive 
                SET ring_number=%s, color_mutations=%s, gender=%s, reason=%s, archive_date=%s, notes=%s
                WHERE id=%s
            """,
          (
              data["ring_number"],
  ...
  def delete_archive(self, archive_id):
    ...
    if reply == QMessageBox.Yes:
      conn = get_connection()
      cursor = conn.cursor()
      cursor.execute("DELETE FROM archive WHERE id=%s", (archive_id,))
      conn.commit()
      conn.close()
      self.load_archive()