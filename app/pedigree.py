...
from app.services import get_cached_image_path
...
class BirdNodeWidget(QFrame):
  ...
  def __init__(self, title, ring="", color="", photo_path=""):
    ...
    lbl_title.setStyleSheet(
        "color: #3182CE; font-weight: bold; font-size: 11px;"
    )
  ...
  def update_data(self, ring, color, photo_path):
    ...
    cached_p = get_cached_image_path(photo_path) if photo_path else ""

    if cached_p and os.path.exists(cached_p):
      pix = QPixmap(cached_p).scaled(
      ...
      self.lbl_photo.setStyleSheet(
          "color: #718096; font-size: 9px; background-color: #1A202C;"
      )
...
class PedigreePage(QWidget):
  ...
  def fetch_bird_details(self, ring_number, conn):
    ...
    cursor.execute(
        "SELECT ring_number, color, NULL as image_path, father_ring,"
        " mother_ring, pair_number FROM chicks WHERE ring_number = %s",
        (ring_number,),
    )
    b = cursor.fetchone()
    if b:
    ...
    cursor.execute(
        "SELECT ring_number, color, image_path, dna_path, father_ring,"
        " mother_ring, NULL as pair_number FROM individual_birds WHERE"
        " ring_number = %s",
        (ring_number,),
    )
    b = cursor.fetchone()
    if b:
      res = dict(b)
      if not res.get("image_path") and res.get("dna_path"):
        res["image_path"] = res["dna_path"]
      return res

    # 3. البحث كذكر في جدول الأزواج
    cursor.execute(
        "SELECT male_ring as ring_number, male_color as color, image_path, NULL"
        " as father_ring, NULL as mother_ring, NULL as pair_number FROM pairs"
        " WHERE male_ring = %s",
        (ring_number,),
    )
    b = cursor.fetchone()
    ...
    cursor.execute(
        "SELECT female_ring as ring_number, female_color as color, image_path,"
        " NULL as father_ring, NULL as mother_ring, NULL as pair_number FROM"
        " pairs WHERE female_ring = %s",
        (ring_number,),
    )
    b = cursor.fetchone()
    if b:
  ...
  def get_parents_rings(self, bird_info, conn):
    ...
    if not father_ring and not mother_ring and bird_info.get("pair_number"):
      cursor = conn.cursor()
      cursor.execute(
          "SELECT male_ring, female_ring FROM pairs WHERE pair_number = %s",
          (bird_info["pair_number"],),
      )
  ...
  def display_tree(self):
    ...
    if target_bird:
      self.node_target.update_data(
          target_bird["ring_number"],
          target_bird["color"],
          target_bird.get("image_path"),
      )
      ...
      father_bird = (
          self.fetch_bird_details(f_ring, conn) if f_ring else None
      )
      mother_bird = (
          self.fetch_bird_details(m_ring, conn) if m_ring else None
      )

      if father_bird:
        self.node_father.update_data(
            father_bird["ring_number"],
            father_bird["color"],
            father_bird.get("image_path"),
        )
      else:
        self.node_father.update_data("", "", "")
      ...
      if mother_bird:
        self.node_mother.update_data(
            mother_bird["ring_number"],
            mother_bird["color"],
            mother_bird.get("image_path"),
        )
      else:
        self.node_mother.update_data("", "", "")

      # الجيل 3: أجداد الأب
      pat_gfather_ring, pat_gmother_ring = (
          self.get_parents_rings(father_bird, conn)
          if father_bird
          else (None, None)
      )
      pat_gf = (
          self.fetch_bird_details(pat_gfather_ring, conn)
          if pat_gfather_ring
          else None
      )
      pat_gm = (
          self.fetch_bird_details(pat_gmother_ring, conn)
          if pat_gmother_ring
          else None
      )
      ...
      if pat_gf:
        self.node_pat_gfather.update_data(
            pat_gf["ring_number"], pat_gf["color"], pat_gf.get("image_path")
        )
      else:
        self.node_pat_gfather.update_data("", "", "")
      ...
      if pat_gm:
        self.node_pat_gmother.update_data(
            pat_gm["ring_number"], pat_gm["color"], pat_gm.get("image_path")
        )
      else:
        self.node_pat_gmother.update_data("", "", "")

      # الجيل 3: أجداد الأم
      mat_gfather_ring, mat_gmother_ring = (
          self.get_parents_rings(mother_bird, conn)
          if mother_bird
          else (None, None)
      )
      mat_gf = (
          self.fetch_bird_details(mat_gfather_ring, conn)
          if mat_gfather_ring
          else None
      )
      mat_gm = (
          self.fetch_bird_details(mat_gmother_ring, conn)
          if mat_gmother_ring
          else None
      )
      ...
      if mat_gf:
        self.node_mat_gfather.update_data(
            mat_gf["ring_number"], mat_gf["color"], mat_gf.get("image_path")
        )
      else:
        self.node_mat_gfather.update_data("", "", "")
      ...
      if mat_gm:
        self.node_mat_gmother.update_data(
            mat_gm["ring_number"], mat_gm["color"], mat_gm.get("image_path")
        )
      else:
        self.node_mat_gmother.update_data("", "", "")
