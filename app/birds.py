...
from PySide6.QtWidgets import (
    ...
    QFormLayout, QLineEdit, QComboBox, QFileDialog, QInputDialog, QFrame
)
...
class DnaCertificateDialog(QDialog):
    def __init__(self, bird_data, parent=None):
        ...
        self.setWindowTitle(f"شهادة DNA - الطير [{bird_data.get('ring_number', '')}]")
    ...
    def init_ui(self):
        ...
        header_lbl.setStyleSheet("font-size: 20px; font-weight: bold; color: #38A169; qproperty-alignment: AlignCenter;")
        card_layout.addWidget(header_lbl)

        # عرض صورة الشهادة
        dna_img_lbl = QLabel()
        dna_img_lbl.setFixedSize(420, 240)
        dna_img_lbl.setStyleSheet("border: 1px dashed #4A5568; border-radius: 8px; background-color: #2D3748;")
        dna_img_lbl.setAlignment(Qt.AlignCenter)

        dna_path = self.bird_data.get('dna_path', '') or self.bird_data.get('image_path', '')
        if dna_path and os.path.exists(dna_path):
            dna_img_lbl.setPixmap(QPixmap(dna_path).scaled(420, 240, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
            dna_img_lbl.setText("لم يتم إرفاق صورة شهادة DNA لهذا الطير")
            dna_img_lbl.setStyleSheet("color: #A0AEC0; font-size: 13px; border: 1px dashed #4A5568; background-color: #2D3748;")
        ...
        btn_export.setStyleSheet("background-color: #38A169; color: white; font-weight: bold; padding: 10px; border-radius: 6px;")
        btn_export.clicked.connect(self.export_as_image)

        btn_close = QPushButton("إغلاق")
        btn_close.setStyleSheet("background-color: #E53E3E; color: white; padding: 10px; border-radius: 6px;")
    ...
    def export_as_image(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "حفظ شهادة DNA", f"DNA_{self.bird_data.get('ring_number', 'bird')}.png", "Images (*.png)"
        )
        if file_path:
            pixmap = self.cert_card.grab()
            pixmap.save(file_path, "PNG")
            QMessageBox.information(self, "تم التصدير", "تم تصدير وتنزيل الشهادة بنجاح!")
...
class AddBirdDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.combo_gender.addItems(db_genders if db_genders else ["ذكر", "أنثى", "بانتظار DNA", "غير معروف"])
        ...
        self.combo_status.addItems(["متاح", "مجهز للتزويج", "للبيع", "تم البيع", "نافق"])
        ...
        self.btn_select_dna.setStyleSheet("background-color: #3182CE; color: white;")
    ...
    def select_dna(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "اختر شهادة DNA", "", "Images (*.png *.jpg *.jpeg *.webp)")
        if file_path:
    ...
    def save_bird(self):
        ...
        if color_val == "اختر اللون...": color_val = ""
        ...
        try:
            ...
            QMessageBox.warning(self, "خطأ", f"رقم الحجل ({ring_num}) مسجل مسبقاً!")
...
class EditBirdDialog(QDialog):
    def __init__(self, bird_data, parent=None):
        super().__init__(parent)
        self.bird_data = bird_data
        self.setWindowTitle(f"تعديل الطير ({bird_data.get('ring_number', '')})")
        self.setFixedSize(420, 530)
        self.setLayoutDirection(Qt.RightToLeft)
        
        self.current_dna_path = bird_data.get('dna_path') or bird_data.get('image_path') or ""
    ...
    def init_ui(self):
        ...
        db_genders = [row['value'] for row in gender_rows]
        self.combo_gender.addItems(db_genders if db_genders else ["ذكر", "أنثى", "بانتظار DNA", "غير معروف"])
        self.combo_gender.setCurrentText(str(self.bird_data.get('gender') or 'غير معروف'))
        ...
        self.input_mutations = QLineEdit(str(self.bird_data.get('mutations') or ''))

        self.combo_status = QComboBox()
        self.combo_status.addItems(["متاح", "مجهز للتزويج", "للبيع", "تم البيع", "نافق"])
        self.combo_status.setCurrentText(str(self.bird_data.get('status') or 'متاح'))
        ...
        dna_name = os.path.basename(self.current_dna_path) if self.current_dna_path else "لم تتم الإضافة"
    ...
    def update_bird(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "خطأ", f"رقم الحجل ({ring_num}) مستخدم لطير آخر!")
        except Exception as err:
...
class BirdsPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.txt_search.setPlaceholderText("🔍 بحث برقم الحجل، اللون، الطفرات، أو المصدر...")
        ...
        self.table.setHorizontalHeaderLabels([
            "id", "شهادة DNA", "رقم الحجل", "الجنس", "اللون", "الطفرات", "الحالة", "المصدر", "إجراءات"
        ])
        ...
        self.table.setColumnWidth(8, 270)
    ...
    def load_filter_options(self):
        ...
        if genders:
            ...
            self.combo_filter_gender.addItems(["ذكر", "أنثى", "بانتظار DNA", "غير معروف"])

        self.combo_filter_status.addItems(["متاح", "مجهز للتزويج", "للبيع", "تم البيع", "نافق"])

        cursor.execute("SELECT value FROM settings_options WHERE category = 'color'")
    ...
    def load_birds(self):
        ...
        selected_status = self.combo_filter_status.currentText() if hasattr(self, 'combo_filter_status') else "جميع الحالات"
        selected_color = self.combo_filter_color.currentText() if hasattr(self, 'combo_filter_color') else "جميع الألوان"
        ...
        try:
            ...
            try:
                cursor.execute("ALTER TABLE individual_birds ADD COLUMN dna_path TEXT")
                conn.commit()
            ...
            if search_text:
                query += " AND (ring_number LIKE ? OR color LIKE ? OR mutations LIKE ? OR source LIKE ?)"
        ...
        for row_idx, row in enumerate(rows):
            ...
            self.table.setItem(row_idx, 0, QTableWidgetItem(str(bird_dict['id'])))
            ...
            dna_p = bird_dict.get('dna_path') or bird_dict.get('image_path') or ""
            ...
            self.table.setItem(row_idx, 2, QTableWidgetItem(str(bird_dict['ring_number'])))
            self.table.setItem(row_idx, 3, QTableWidgetItem(str(bird_dict.get('gender') or '')))
            self.table.setItem(row_idx, 4, QTableWidgetItem(str(bird_dict.get('color') or '')))
            self.table.setItem(row_idx, 5, QTableWidgetItem(str(bird_dict.get('mutations') or '')))
            self.table.setItem(row_idx, 6, QTableWidgetItem(str(bird_dict.get('status') or '')))
            self.table.setItem(row_idx, 7, QTableWidgetItem(str(bird_dict.get('source') or '')))
            ...
            btn_cert.setStyleSheet("background-color: #38A169; color: white; border-radius: 4px; padding: 4px 6px; font-weight: bold;")
            btn_cert.clicked.connect(lambda checked, data=bird_dict: self.show_dna_certificate(data))

            btn_edit = QPushButton("تعديل")
            btn_edit.setStyleSheet("background-color: #3182CE; color: white; border-radius: 4px; padding: 4px 8px;")
            btn_edit.clicked.connect(lambda checked, data=bird_dict: self.edit_bird(data))

            btn_archive = QPushButton("أرشفة")
            btn_archive.setStyleSheet("background-color: #DD6B20; color: white; border-radius: 4px; padding: 4px 8px; font-weight: bold;")
            btn_archive.clicked.connect(lambda checked, ring=bird_dict['ring_number'], color=str(bird_dict.get('color') or ''), gender=str(bird_dict.get('gender') or ''): self.archive_bird(ring, color, gender))

            btn_delete = QPushButton("حذف")
            btn_delete.setStyleSheet("background-color: #E53E3E; color: white; border-radius: 4px; padding: 4px 8px;")
            btn_delete.clicked.connect(lambda checked, b_id=bird_dict['id'], ring=bird_dict['ring_number']: self.delete_bird(b_id, ring))
    ...
    def archive_bird(self, ring_number, color, gender):
        ...
        if confirm == QMessageBox.Yes:
            ...
            reason, ok = QInputDialog.getItem(self, "سبب الأرشفة", "اختر سبب نقل الطير للأرشيف:", reasons, 0, False)
            ...
            if ok and reason:
                ...
                try:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (ring_number, color, gender, reason, today_str, "تم النقل من قائمة الطيور"))
                    
                    cursor.execute("DELETE FROM individual_birds WHERE ring_number = ?", (ring_number,))
                    cursor.execute("DELETE FROM chicks WHERE ring_number = ?", (ring_number,))
                    
                    conn.commit()
                    QMessageBox.information(self, "تمت الأرشفة", f"تم نقل الطير [{ring_number}] بنجاح إلى صفحة الأرشيف.")
                finally:
                    if conn: conn.close()
    ...
    def delete_bird(self, bird_id, ring_number):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                conn = get_connection()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM individual_birds WHERE id=?", (bird_id,))
                conn.commit()
            finally:
                if conn: conn.close()
            self.load_birds()