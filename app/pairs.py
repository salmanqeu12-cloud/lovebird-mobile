...
from app.services import (
    ...
    get_settings_options_by_category,
)
...
class AddPairDialog(QDialog):
  ...
  def select_image(self):
    file_path, _ = QFileDialog.getOpenFileName(
        self, "اختر صورة الزوج", "", "Images (*.png *.jpg *.jpeg *.webp)"
    )
    if file_path:
  ...
  def save_pair(self):
    ...
    if self.image_path and os.path.exists(self.image_path):
      saved_img_path = compress_and_upload_image(
          self.image_path, prefix=f"pair_{pair_num}"
      )
    ...
    try:
      ...
      if male_ring:
        ...
        if cursor.fetchone():
          cursor.execute(
              "UPDATE individual_birds SET gender='ذكر', color=%s, status='في"
              " زوج', source=%s WHERE ring_number=%s",
              (male_color, f"زوج برقم {pair_num}", male_ring),
          )
        else:
          cursor.execute(
              "INSERT INTO individual_birds (ring_number, gender, color,"
              " status, source) VALUES (%s, 'ذكر', %s, 'في زوج', %s)",
      ...
      if female_ring:
        ...
        if cursor.fetchone():
          cursor.execute(
              "UPDATE individual_birds SET gender='أنثى', color=%s,"
              " status='في زوج', source=%s WHERE ring_number=%s",
              (female_color, f"زوج برقم {pair_num}", female_ring),
          )
        else:
          cursor.execute(
              "INSERT INTO individual_birds (ring_number, gender, color,"
              " status, source) VALUES (%s, 'أنثى', %s, 'في زوج', %s)",
      ...
      if "unique" in str(err).lower() or "duplicate" in str(err).lower():
        QMessageBox.warning(
            self, "خطأ", f"رقم الزوج ({pair_num}) موجود مسبقاً!"
        )
      else:
...
class EditPairDialog(QDialog):
  ...
  def __init__(self, pair_data, parent=None):
    ...
    self.setLayoutDirection(Qt.RightToLeft)
    self.image_path = (
        pair_data["image_path"]
        if "image_path" in pair_data.keys() and pair_data["image_path"]
        else ""
    )
    self.init_ui()
  ...
  def init_ui(self):
    ...
    self.input_female_ring = QLineEdit(
        str(self.pair_data["female_ring"] or "")
    )

    self.combo_male_color = QComboBox()
    self.combo_male_color.addItems(colors_list)
    self.combo_male_color.setEditable(True)
    self.combo_male_color.setCurrentText(
        str(self.pair_data["male_color"] or "")
    )

    self.combo_female_color = QComboBox()
    self.combo_female_color.addItems(colors_list)
    self.combo_female_color.setEditable(True)
    self.combo_female_color.setCurrentText(
        str(self.pair_data["female_color"] or "")
    )
  ...
  def update_pair(self):
    ...
    new_male_ring = self.input_male_ring.text().strip()
    new_female_ring = self.input_female_ring.text().strip()
    old_male_ring = str(self.pair_data.get("male_ring") or "").strip()
    old_female_ring = str(self.pair_data.get("female_ring") or "").strip()

    saved_img_path = self.image_path
    if (
        self.image_path
        and os.path.exists(self.image_path)
        and self.image_path != self.pair_data.get("image_path")
    ):
      cloud_url = compress_and_upload_image(
          self.image_path, prefix=f"pair_{pair_num}"
      )
      if cloud_url:
        saved_img_path = cloud_url
    ...
    try:
      ...
      def sync_bird(old_ring, new_ring, gender, color):
            ...
            if new_ring:
              cursor.execute(
                  """
                                ...
                                SET ring_number = %s, color = %s, gender = %s, status = 'في زوج', source = %s
                                WHERE ring_number = %s
                            """,
                  (
                      new_ring,
              ...
              cursor.execute(
                  "UPDATE individual_birds SET status = 'فردي', source = ''"
                  " WHERE ring_number = %s",
                  (old_ring,),
              )
            return
        ...
        if new_ring:
          ...
          if cursor.fetchone():
            cursor.execute(
                """
                            UPDATE individual_birds 
                            SET color = %s, gender = %s, status = 'في زوج', source = %s
                            WHERE ring_number = %s
                        """,
                (color, gender, f"زوج برقم {pair_num}", new_ring),
            )
          else:
            cursor.execute(
                """
                            INSERT INTO individual_birds (ring_number, gender, color, status, source)
                            VALUES (%s, %s, %s, 'في زوج', %s)
                        """,
                (new_ring, gender, color, f"زوج برقم {pair_num}"),
            )

      sync_bird(old_male_ring, new_male_ring, "ذكر", male_color)
      sync_bird(old_female_ring, new_female_ring, "أنثى", female_color)
      ...
      if "unique" in str(err).lower() or "duplicate" in str(err).lower():
        QMessageBox.warning(
            self,
            "خطأ",
            f"رقم الزوج ({pair_num}) أو رقم الحجل مستخدم لزوج آخر!",
        )
      else:
        QMessageBox.critical(
...
class PairsPage(QWidget):
  ...
  def init_ui(self):
    ...
    self.txt_search.setPlaceholderText(
        "🔍 بحث برقم الزوج، حجل الذكر، حجل الأنثى، أو اللون..."
    )
    ...
    # مقاسات ثابتة ومريحة للأعمدة لمنع تداخل النصوص عند تصغير الشاشة
    self.table.setColumnWidth(1, 80)   # الصورة
    self.table.setColumnWidth(2, 75)   # رقم الزوج
    self.table.setColumnWidth(3, 85)   # حجل الذكر
    self.table.setColumnWidth(4, 85)   # حجل الأنثى
    self.table.setColumnWidth(5, 140)  # لون الذكر
    self.table.setColumnWidth(6, 140)  # لون الأنثى
    self.table.setColumnWidth(7, 75)   # عدد البطون
    self.table.setColumnWidth(8, 75)   # عدد الفروخ
    self.table.setColumnWidth(9, 75)   # الحالة
    self.table.setColumnWidth(10, 110) # ملاحظات
    self.table.setColumnWidth(11, 145) # إجراءات
  ...
  def load_pairs(self):
    ...
    try:
      ...
      if search_query:
        query += """
                    WHERE p.pair_number LIKE %s 
                       OR p.male_ring LIKE %s 
                       OR p.female_ring LIKE %s 
                       OR p.male_color LIKE %s 
                       OR p.female_color LIKE %s 
                """
    ...
    for row_idx, row in enumerate(rows):
      ...
      cached_p = get_cached_image_path(img_p) if img_p else ""

      if cached_p and os.path.exists(cached_p):
        pixmap = QPixmap(cached_p)
      ...
      get_val = (
          lambda key, default="": str(row[key])
          if key in row.keys() and row[key] is not None
          else default
      )

      self.table.setItem(
          row_idx, 2, QTableWidgetItem(get_val("pair_number"))
      )
      self.table.setItem(row_idx, 3, QTableWidgetItem(get_val("male_ring")))
      self.table.setItem(
          row_idx, 4, QTableWidgetItem(get_val("female_ring"))
      )
      self.table.setItem(
          row_idx, 5, QTableWidgetItem(get_val("male_color"))
      )
      self.table.setItem(
          row_idx, 6, QTableWidgetItem(get_val("female_color"))
      )
      self.table.setItem(
          row_idx, 7, QTableWidgetItem(get_val("clutch_count", "0"))
      )
      self.table.setItem(
          row_idx, 8, QTableWidgetItem(get_val("chick_count", "0"))
      )
      ...
      btn_edit.clicked.connect(
          lambda checked, r=row: self.edit_pair(dict(r))
      )
      btn_delete.clicked.connect(
          lambda checked, r_id=row["id"], p_num=row[
              "pair_number"
          ]: self.delete_pair(r_id, p_num)
      )
  ...
  def delete_pair(self, pair_id, pair_number):
    ...
    if reply == QMessageBox.Yes:
      ...
      try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM pairs WHERE id=%s", (pair_id,))
        conn.commit()
      finally:
        if conn:
          conn.close()
